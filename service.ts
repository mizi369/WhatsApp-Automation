...
function startWhatsApp() {
  ...
  client.on('qr', (qr) => {
    console.log('[WHATSAPP] QR Received, converting to DataURL...');
    qrcode.toDataURL(qr, { scale: 10 }).then(url => {
        io.emit('qr-code', url);
        io.emit('stage-update', 'SCAN_QR');
        console.log('[WHATSAPP] QR Code sent to client');
    }).catch(err => {
...
io.on('connection', (socket) => {
    ...
    // NEW: Full Reset (Clears Auth Folder)
    socket.on('cmd-full-reset', async () => {
        console.log('[SYSTEM] Full Reset Requested (Clearing Auth)');
        if (client) {
            try {
                await client.destroy();
            } catch (e) {
                console.error('[WHATSAPP] Destroy Error during reset:', e.message);
            }
            client = null;
        }
        ...
        if (fs.existsSync(authPath)) {
            try {
                ...
                console.log('[SYSTEM] Auth folder cleared');
                io.emit('sys-log', { time: new Date().toLocaleTimeString(), msg: 'Session cleared successfully. Restarting...', type: 'success' });
            } catch (e) {
                console.error('[SYSTEM] Failed to clear auth folder:', e.message);
                io.emit('sys-log', { time: new Date().toLocaleTimeString(), msg: `Failed to clear session: ${e.message}`, type: 'error' });
            }
        }
