  ...
  const syncLiveSlotsToAi = async () => {
      try {
          const getAvailabilityForDate = async (dateStr: string) => {
              ...
              const dateBookings = bookings.filter(b => (b.booking_date || (b as any).date) === dateStr);
              ...
              return slots.filter((s: any) => s.active).map((s: any) => {
                  ...
                  activeTeams.forEach((team: any) => {
                      ...
                      const teamBookings = bookings.filter(b => ((b.booking_date || (b as any).date) === dateStr) && (b.team === teamIdStr || b.team === team.name || b.team_id?.toString() === teamIdStr || (b as any).teamId?.toString() === teamIdStr));
                      const teamDailyCount = teamBookings.length;
                      const teamAircondCount = teamBookings.filter(b => (b.service_type || (b as any).serviceType || '').toLowerCase().includes('pasang')).length;
                      const teamHasSlotBooking = slotBookings.some(b => b.team === teamIdStr || b.team === team.name || b.team_id?.toString() === teamIdStr || (b as any).teamId?.toString() === teamIdStr);
          ...
          const todayStr = `TARIKH HARI INI (${today}):\n${todayStatus.map((s: any) => `${s.label || s.time} [SlotID:${s.id}]: ${s.label}`).join('\n')}`;
          const tomorrowStr = `TARIKH ESOK (${tomorrow}):\n${tomorrowStatus.map((s: any) => `${s.label || s.time} [SlotID:${s.id}]: ${s.label}`).join('\n')}`;
  ...
  useEffect(() => {
    ...
    const handleAutoBooking = async (data: any) => {
        ...
        const newBooking: Booking = {
            ...
            customer_name: data.name || data.customer_name || (data as any).customerName || 'Pelanggan WhatsApp',
            address: data.address || 'Alamat tidak dinyatakan',
            phone: data.phone || '0123456789',
            service_type: data.service || data.service_type || (data as any).serviceType || 'Servis Aircond',
            unit_type: data.unit || data.unit_type || (data as any).unitType || '1.0 HP',
            quantity: data.quantity || data.unit || '1',
            time_slot: data.time || data.time_slot || (data as any).timeSlot || '9:00 AM – 11:00 AM',
            team: teamName,
        ...
        try {
            ...
            const isDuplicate = currentBookings.some(b => 
                (b.id === newBooking.id) || 
                (b.phone === newBooking.phone && (b.booking_date || (b as any).date) === newBooking.booking_date && (b.time_slot || (b as any).timeSlot) === newBooking.time_slot)
            );
            ...
            if (!isDuplicate) {
                ...
                showToast(`Tempahan Baru: ${newBooking.customer_name} pada ${newBooking.booking_date} (${newBooking.time_slot})`, 'success');
  ...
  return (
    ...
    >
      <div className="flex h-screen bg-darker text-slate-100 font-sans overflow-hidden">
        ...
        {/* --- SIDEBAR --- */}
        <div className={`transition-all duration-300 ease-in-out border-r border-slate-800 bg-slate-900 flex flex-col z-50 ${isSidebarOpen ? 'w-64 translate-x-0' : 'w-20 -translate-x-full md:translate-x-0'} absolute md:relative h-full shadow-2xl`}>
          
          {/* Logo Section */}
          <div className={`h-20 flex items-center justify-between border-b border-slate-800 relative ${isSidebarOpen ? 'px-6' : 'px-4 md:px-2'}`}>
        ...
        <div className={`flex-1 bg-darker relative overflow-hidden flex flex-col h-full w-full transition-all duration-300 ${isSidebarOpen ? 'md:ml-0' : 'md:ml-0'}`}>
           ...
           <div className="bg-slate-900 border-b border-slate-800 px-4 py-2 flex justify-between items-center z-40 sticky top-0 shadow-md h-14 shrink-0">
               <div className="flex items-center gap-3">
                   {/* Mobile Menu Toggle */}
                   <button 
                      onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                      className="md:hidden text-slate-400 hover:text-white p-2 -ml-2 transition-colors"
                   >
           ...
           <div className={`flex-1 overflow-auto custom-scrollbar p-3 sm:p-4 md:p-5 lg:p-6 ${isSidebarOpen ? 'md:pl-6' : ''} relative`}>
              <AnimatePresence>
                {!isSocketConnected && (
                  <motion.div 
                    initial={{ opacity: 0 }}
