...
const WhatsAppMonitor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Fallback: If status remains CHECKING for > 30s, set to OFFLINE
                  ...
                  <div className="md:w-1/2 p-6 md:p-10 flex flex-col justify-between overflow-y-auto">
                      ...
                      <div className="space-y-4 my-6">
                          ...
                          <div className="flex flex-col gap-2">
                              ...
                              <button 
                                  onClick={() => {
                                      if(confirm("AWAS: Ini akan memadam semua data sesi (Login) WhatsApp. Sila gunakan hanya jika sistem tersangkut. Sahkan 'Deep System Reset'?")) {
                                          socket.emit('cmd-full-reset');
                                          alert('Reset sistem dijalankan. Sila tunggu 5-10 saat untuk QR baru.');
                                      }
                                  }}
                                  className="w-full bg-slate-100 text-slate-500 py-2 rounded-xl font-bold uppercase tracking-widest text-[8px] hover:bg-red-50 hover:text-red-500 transition-all flex items-center justify-center gap-2 border border-slate-200 shadow-sm active:scale-95"
                              >
                                  <RefreshCw size={10} /> Deep System Reset (Railway Fix)
                              </button>
                          </div>
                      </div>
