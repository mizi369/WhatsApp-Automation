...
export default function Booking({ showToast }: { showToast: (msg: string, type?: 'success' | 'error' | 'info') => void }) {
  ...
  const fetchAvailability = async (date: string) => {
    try {
      ...
      const availabilityData: TeamAvailability[] = teams.filter((t: any) => t.is_active === true).map((team: any) => {
        const teamBookings = dateBookings.filter((b: any) => (b.team_id || b.teamId || b.team)?.toString() === team.id.toString() || b.team === team.name);
        const aircondJobs = teamBookings.filter((b: any) => {
          const service = b.service_type || b.serviceType || '';
          return service.toLowerCase().includes('pasang');
        }).length;
        const maxAircondJobs = team.max_install_jobs || 2;
        const maxJobs = team.max_jobs_per_day || 4;
        
        const isAircondFull = aircondJobs >= maxAircondJobs;

        const teamSlots = slots.map((slot: any) => {
          const slotBookings = teamBookings.filter((b: any) => (b.time_slot_id || b.slotId || b.time_slot || b.timeSlot)?.toString() === slot.id.toString() || b.time_slot === slot.label || b.timeSlot === slot.label);
          
          const isSlotActive = slot.is_active === true;
          const isBlocked = dateBlockedSlots.some((b: any) => b.time_slot === slot.label || b.timeSlot === 'ALL');
          ...
          const allowedSlots = team.allowed_slots || [];
          if (available && allowedSlots && allowedSlots.length > 0 && !allowedSlots.includes(slot.id)) {
            available = false;
            reason = 'Slot tidak dibenarkan untuk team ini';
          }

          if (available && formData.service_type && formData.service_type.toLowerCase().includes('pasang') && isAircondFull) {
             available = false;
  ...
  const handleAddBooking = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.team_id || !formData.time_slot_id) {
    ...
    try {
      ...
      const isDuplicate = bookings.some(b => 
        (b.phone === formData.phone && (b.booking_date || b.date) === selectedDate && (b.time_slot || b.timeSlot) === (availability.find(t => t.teamId.toString() === formData.team_id.toString())?.slots.find(s => s.slotId.toString() === formData.time_slot_id.toString())?.label || ''))
      );
      ...
      const selectedTeam = availability.find(t => t.teamId.toString() === formData.team_id.toString());
      const selectedSlot = selectedTeam?.slots.find(s => s.slotId.toString() === formData.time_slot_id.toString());

      const newBooking = {
        ...formData,
        booking_date: selectedDate,
        team_id: selectedTeam?.teamId.toString() || '',
            ...
            <div className="grid grid-cols-7 gap-1">
              {(() => {
                ...
                return [
                  ...
                  const blockedSlotsRaw = db.getAll<any>(TABLES.BLOCKED_SLOTS) || [];
                  const isHoliday = day ? blockedSlotsRaw.some((b: any) => b.date === dateStr && (b.time_slot === 'ALL' || b.reason === 'CLOSE_ALL')) : false;
                          ...
                          {inventoryItems.length > 0 && (
                            <optgroup label="Inventori" className="bg-dark">
                              {inventoryItems.map(item => (
                                <option key={item.id} value={item.item_name} className="bg-dark">
                                  {item.item_name}
                                </option>
                              ))}
                            </optgroup>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1">
                        ...
                        <select 
                          ...
                          onChange={(e) => setFormData({...formData, team_id: e.target.value, time_slot_id: ''})}
                          className="w-full bg-darker border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:border-secondary outline-none text-white"
                        >
                      ...
                      <div className="space-y-1">
                        ...
                        >
                          ...
                          {formData.team_id && availability.find(t => t.teamId.toString() === formData.team_id)?.slots.map(slot => (
                      ...
                      <button 
                        ...
                        onClick={() => {
                          ...
                          if (availableSlot) {
                            setFormData({
                              ...formData,
                              team_id: availableSlot.teamId.toString(),
                              time_slot_id: availableSlot.slotId.toString()
                            });
