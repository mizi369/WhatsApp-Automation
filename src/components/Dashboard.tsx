...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 max-w-7xl mx-auto pb-8">
                            ...
                            <div key={job.id} className="px-4 py-2.5 flex items-center justify-between hover:bg-white/[0.02] transition-colors cursor-default group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="text-xs font-black text-white uppercase truncate">{job.customer_name || (job as any).customerName}</p>
                                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wide truncate">{(job.service_type || (job as any).serviceType)} • {job.quantity}</p>
                                    </div>
                                </div>
                                <div className="text-right shrink-0">
                                    ...
                                    <p className="text-[9px] text-slate-500 font-bold mt-0.5">{job.time_slot || (job as any).timeSlot}</p>
                                </div>
                            </div>
                        ))
                    )}
...
const MiniStat: React.FC<{icon: any, label: string, value: string, color: 'cyan'|'indigo'|'emerald'|'orange'}> = ({icon, label, value, color}) => {
    ...
    return (
        <div className={`p-3 rounded-xl border ${colors[color]} flex flex-col justify-between h-24 relative overflow-hidden group hover:shadow-[0_0_15px_rgba(0,188,212,0.6)] transition-all glass-panel cursor-default`}>
            ...
            <div className="flex items-center gap-1.5 mb-1 relative z-10 w-full overflow-hidden">
                <div className="p-1.5 bg-white/10 rounded-lg shadow-sm backdrop-blur-md border border-white/5 shrink-0">{icon}</div>
                <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest opacity-80 truncate">{label}</span>
            </div>
